...
const encryptionKeyRegex = new RegExp(`^[0-9a-f]{${_index.ENCRYPTION_PARAMETERS.keyLength / 4}}$`);
describe("storage/crypto", () => {
  it("should encrypt and decrypt unicode symbols correctly", () => {
    const originalObject = JSON.stringify({
      data: "Rating(≤ 25kg)"
    });
    const enc = (0, _index.encryptString)(originalObject);
    const dec = (0, _index.decryptString)(enc);
  ...
  describe("encryptString", () => {
    ...
    test("should have the right keys and values when no key passed", /*#__PURE__*/_asyncToGenerator(function* () {
      encryptionResults = (0, _index.encryptString)("hello world");
      expect(encryptionResults).toStrictEqual(expect.objectContaining({
        ...
        type: _index.ENCRYPTION_PARAMETERS.version
      }));
    }));
    test("should have the right keys and values when key is passed", /*#__PURE__*/_asyncToGenerator(function* () {
      const encryptionKey = "35fb46ca758889669f38c83d2f159b0f5a320b5a97387a9eaecb5652d15e0e3d";
      encryptionResults = (0, _index.encryptString)("hello world", encryptionKey);
      expect(encryptionResults).toStrictEqual(expect.objectContaining({
        cipherText: expect.stringMatching(base64Regex),
        iv: expect.stringMatching(base64Regex),
        tag: expect.stringMatching(base64Regex),
    ...
    test("should throw error if input is not a string", () => {
      encryptionResults = (0, _index.encryptString)("hello world"); // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore because we're explicitly testing failure mode

      expect(() => (0, _index.encryptString)({})).toThrow("encryptString only accepts strings"); // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore because we're explicitly testing failure mode

      expect(() => (0, _index.encryptString)(2)).toThrow("encryptString only accepts strings");
    });
  });
  describe("decryptString", () => {
    test("can decrypt", () => {
      const encryptionResults = (0, _index.encryptString)("hello world");
      expect((0, _index.decryptString)(encryptionResults)).toBe("hello world");
    });
    test("can decrypt when encryption key is passed", () => {
      const encryptionKey = "35fb46ca758889669f38c83d2f159b0f5a320b5a97387a9eaecb5652d15e0e3d";
      const encryptionResults = (0, _index.encryptString)("hello world", encryptionKey);
      expect((0, _index.decryptString)(encryptionResults)).toBe("hello world");
    });
  });
  describe("encodeDocument & decodeDocument", () => {
    it("should do the reverse of each other", () => {
      const input = "hello";
      const encoded = (0, _index.encodeDocument)(input);
      const decoded = (0, _index.decodeDocument)(encoded);
