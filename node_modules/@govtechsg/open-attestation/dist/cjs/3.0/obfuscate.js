...
var obfuscate = function (_data, fields) {
    var data = (0, lodash_1.cloneDeep)(_data); // Prevents alteration of original data
    ...
    var fieldsToRemove = (0, traverseAndFlatten_1.traverseAndFlatten)((0, lodash_1.pick)(data, fieldsAsArray), {
        iteratee: function (_a) {
            var path = _a.path;
            return path;
        },
    });
    var salts = (0, salt_1.decodeSalt)(data.proof.salts);
    // Obfuscate data by hashing them with the key
    var obfuscatedData = fieldsToRemove.map(function (field) {
        var _a;
        var value = (0, lodash_1.get)(data, field);
        var salt = salts.find(function (s) { return s.path === field; });
        if (!salt) {
            throw new Error("Salt not found for " + field);
        }
        return (0, utils_1.toBuffer)((_a = {}, _a[salt.path] = salt.value + ":" + value, _a)).toString("hex");
    });
    // remove fields from the object
    fieldsAsArray.forEach(function (field) { return (0, lodash_1.unset)(data, field); });
    data.proof.salts = (0, salt_1.encodeSalt)(salts.filter(function (s) { return !fieldsToRemove.includes(s.path); }));
    return {
        data: data,
